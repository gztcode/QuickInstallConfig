...
CLUSTER_CONF="${CONF_DIR}/redis-cluster-${REDIS_PORT}.conf"  

if [ -e "${REDIS_CONF}" ]; then
    echo "文件存在"
    cd "${CONF_DIR}"
    cp redis.conf "${CLUSTER_CONF}"
else
    # 检查依赖  
    ...
    chown -R ${REDIS_USER}:${REDIS_USER} "${DATA_DIR}" "${LOG_DIR}" ""${CONF_DIR}""  
...
# 修改CLUSTER_CONF配置文件中的一些默认设置（可选）  
sed -i "s|^dir .*|dir $DATA_DIR|" "${CLUSTER_CONF}"
sed -i "s|^logfile .*|logfile \"$LOG_DIR/redis-cluster-$REDIS_PORT.log\"|" "${CLUSTER_CONF}"
sed -i "s|^dbfilename .*|dbfilename dump-cluster-$REDIS_PORT.rdb|" "${CLUSTER_CONF}"
sed -i "s|^port .*|port $REDIS_PORT|" "${CLUSTER_CONF}"
sed -i "s|^pidfile .*|pidfile /var/run/redis-cluster-$REDIS_PORT.pid|" "${CLUSTER_CONF}"
sed -i 's/^daemonize no$/daemonize yes/' "${CLUSTER_CONF}"
sed -i 's/^protected-mode yes$/protected-mode no/' "${CLUSTER_CONF}"
sed -i 's/^bind/# bind/' "${CLUSTER_CONF}"
sed -i "s|^\# cluster-enabled yes|cluster-enabled yes|" "${CLUSTER_CONF}"
sed -i "s|^# cluster-config-file .*|cluster-config-file nodes-$REDIS_PORT.conf|" "${CLUSTER_CONF}"
sed -i "s|^# cluster-node-timeout .*|cluster-node-timeout 5000|" "${CLUSTER_CONF}"
...
if [ -n "$REDIS_PASSWORD" ]; then
    sed -i "s|^# requirepass .*|requirepass $REDIS_PASSWORD|" "${CLUSTER_CONF}"
...
echo "Redis Cluster is now running 'redis-server ${CLUSTER_CONF}'"
